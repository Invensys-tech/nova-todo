...
class _AnalyticsViewState extends State<AnalyticsView> {
  ...
  void dispose() {
    ...
    _startDateController.removeListener(_onDateChanged);
    _endDateController.removeListener(_onDateChanged);
  ...
  void _onDateChanged() {
    // This method is called when the text in either controller changes.
    // We should refetch the data here.
    print("Date controller text changed. Refetching expenses...");
    _refreshExpenses();
    // You might want to call setState if the UI needs to reflect the loading state immediately.
    // setState(() {}); // Uncomment this line if you want the UI to rebuild on every date change.
  }
  ...
  void _refreshExpenses() {
    ...
    // Only refetch if both dates are valid
    if (startDate != null && endDate != null) {
      setState(() {
        _expensesFuture = widget.datamanager.getExpense(
          dateTime: startDate,
      ...
      // Handle cases where dates are invalid, maybe show an error or default to a range.
      // For now, let's just print a message.
      print("Invalid date format in controllers.");
      // You might want to keep the old future or set it to an empty list Future
      // depending on your desired behavior when dates are invalid.
    }
  }
              ...
              ExpansionPanelList(
                ...
                children: [
                  ...
                  ExpansionPanel(
                    ...
                    body: FutureBuilder<List<Expense>>(
                      future:
                          _expensesFuture, // This future is updated when dates change
                      builder: (context, snapshot) {
                        if (snapshot.connectionState ==
                            ConnectionState.waiting) {
                          return const Center(
